...
`RequestArb`对来自`MSHR`、`sinkA`、`sinkB`、`sinkC`的请求进行仲裁，分为s0、s1、s2。
- s0，从`MSHR`接收请求。锁存一拍，传入s1。
- s1，仲裁，优先级顺序：`MSHR`>`sinkC`>`sinkB`>`sinkA`
  - 把选出的任务信息发送给`MainPipe`
  - 向`Directory`发送查询
  - 向`RequestBuffer`发送信息
- s2，向`MainPipe`发送信息，读`RefillBuffer`和`ReleaseBuffer`

`MainPipe`是主流水线，分为s1、s2、s3、s4、s5
- s1，接收`RequestArb`的请求
- s2，接收`RequestArb`的请求
- s3，从`Directory`、`RefillBuffer`、`ReleaseBuffer`接收查询结果，向`DataStorage`发送请求
- s4，
